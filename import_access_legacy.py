...
def get_job_mapping(processing_item_id):
    # (job_type_id, job_category, expected_equipment)
    mapping = {
        '1': ('1', 'MOLD_NEW', 'MOLD'),
        '2': ('1', 'MOLD_NEW', 'MOLD'),
...
def main():
    ...
    summary = {
        'source_records': {
            'molds': len(molds_data),
            'cutters': len(cutters_data),
            'designs': len(designs_data),
            'jobs': len(jobs_data),
            'job_steps': len(job_steps_data),
            'worklogs': len(worklogs_data)
        },
    ...
    for row in designs_data:
        ...
        if d_id:
            ...
            valid_designs[legacy_id] = {
                'revision_code': row.get('MoldDesignCode') or f"DES-{d_id}",
                'legacy_id': legacy_id,
    ...
    valid_equipment = {} # legacy_id -> data
    ...
    for row in molds_data:
        ...
        valid_equipment[legacy_id] = {
            ...
            'equipment_name': row.get('MoldName') or "Unknown",
            'equipment_type': eq_type,
            'legacy_id': legacy_id,
            'legacy_specs': json.dumps(row),
            'design_revision_legacy_id': f"D-{design_id}" if design_id else None
        }
    ...
    for row in cutters_data:
        ...
        valid_equipment[legacy_id] = {
            ...
            'equipment_name': row.get('CutterName') or "Unknown Cutter",
    ...
    # Write Artifacts for Dry-Run
    with open(os.path.join(OUTPUT_DIR, 'recovery_dry_run_summary.json'), 'w') as f:
    ...
    if args.execute:
        ...
        try:
            ...
            if args.stage == 'A':
                # STAGE A: design_revisions and equipment
                print(f"Inserting {len(valid_designs)} design_revisions...")
                # Note: ON CONFLICT (legacy_id) requires a UNIQUE constraint on legacy_id.
                # If not present, we create a temporary unique index or just rely on UUID if generating one.
                # Here we assume legacy_id has a unique constraint or we do UPSERT by legacy_id.
                ...
                design_query = """
                    INSERT INTO design_revisions (revision_code, legacy_id, legacy_specs)
                    VALUES %s
                ...
                """
                design_values = [(d['revision_code'], d['legacy_id'], d['legacy_specs']) for d in valid_designs.values()]
                execute_values(cur, design_query, design_values, fetch=True)
                design_rows = cur.fetchall()
                design_legacy_map = {row[1]: row[0] for row in design_rows} # legacy_id -> revision_id UUID
                ...
                eq_query = """
                    INSERT INTO equipment (equipment_code, equipment_name, equipment_type, legacy_id, legacy_specs, design_revision_id)
                    VALUES %s
                    ON CONFLICT (legacy_id) DO UPDATE SET
                ...
                """
                eq_values = [
                    (e['equipment_code'], e['equipment_name'], e['equipment_type'], e['legacy_id'], e['legacy_specs'], design_legacy_map.get(e['design_revision_legacy_id']))
                    for e in valid_equipment.values()
                ]
                execute_values(cur, eq_query, eq_values)
                
            elif args.stage == 'B':
                # STAGE B: work_orders, jobs, etc.
                # ... Similar idempotency logic ...
                print("Stage B implementation requires ensuring Stage A mappings exist.")
                # We would fetch equipment and design mapping, then insert WO and Jobs.
                pass
