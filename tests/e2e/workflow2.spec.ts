...
test.describe('Order Creation Flow', () => {
  test('should create customer and orders', async ({ page }) => {
    ...
    await Promise.all([
      page.waitForResponse(res => res.request().method() === 'POST' && res.url().includes('/master/customers')),
    ...
    const custInput = page.locator('.col-span-1').filter({ hasText: '得意先' }).locator('input[type="text"]').first();
    ...
    console.log('Number of submit buttons:', submitBtns.length);
    for (let i = 0; i < submitBtns.length; i++) {
        console.log(`Submit button ${i} text:`, await submitBtns[i].textContent());
    }
    const dialogPromise = page.waitForEvent('dialog', { timeout: 5000 }).catch(() => null);
    await submitBtns[submitBtns.length - 1].click(); // click the last one, usually save
    const d = await dialogPromise;
    console.log('Dialog result after submit:', d ? d.message() : 'No dialog');
