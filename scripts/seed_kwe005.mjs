...
async function main() {
  ...
  try {
    // 1. Insert customers
    console.log('1. Inserting Customer (KOWA-EMORI)...');
    let customer;
    const { data: extCust } = await supabase.from('customers').select('*').eq('customer_code', 'KOWA-EMORI').maybeSingle();
    if (extCust) {
      customer = extCust;
      console.log(` -> Customer exists: ${customer.id}`);
    } else {
      const { data, error } = await supabase.from('customers').insert({
        customer_code: 'KOWA-EMORI',
    ...
    if (extOrd) {
      const { data, error } = await supabase.from('orders').update({
        order_type: 'design_mold'
      }).eq('id', extOrd.id).select().single();
      if (error) throw error;
      order = data;
