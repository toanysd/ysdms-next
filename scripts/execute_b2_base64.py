...
df = pd.read_excel('./source_data/生産指示書/B. トレイデーク一覧譨.xlsx', header=None, skiprows=4)
...
for idx, row in df.iterrows():
    ...
    plastic_text = f'{zaisitsu} {thickness}t [{width}] 帯雸:{taiden} シリコ:{silicon} 吗市:{tofu}'
...
for i, u in enumerate(updates):
    try:
        res = supabase.table('design_revisions').update({
            ...
            'plastic_type_designed': u['plastic_text_designed']
		#ah wait the column is plastic_type_designed
...
for i in range(min(5, len(updates))):
    print(f'Design: {updates[i]["design_code"]}  |  Text: {updates[i]["plastic_type_designed"]}  |  Family: {updates[i]["plastic_family"]}')
