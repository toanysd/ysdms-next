...
async function syncCuttersToEquipment() {
  console.log('=== SYNCING CUTTERS TO EQUIPMENT TABLE ===')
  ...
  if (!cutters || cutters.length === 0) {
    console.log('No cutters found.')
    return
  }

  console.log(`Found ${cutters.length} cutters to sync...`)
  let updatedCount = 0
  ...
  for (const c of cutters) {
    ...
    const { error } = await supabase
      .from('equipment')
      .update(payload)
      .eq('legacy_cutter_id', c.cutter_id)

    if (!error) updatedCount++
  }

  console.log(`Successfully synced ${updatedCount} / ${cutters.length} cutter equipment rows!`)
}

syncCuttersToEquipment().catch(console.error)
