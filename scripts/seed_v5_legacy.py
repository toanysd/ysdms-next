...
def main():
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        for r in cutters:
            ...
            f.write(f"INSERT INTO public.cutters (cutter_id, cutter_code, cutter_name, pitch, manufacture_date) VALUES ")
            f.write(f"({cutter_uuid(cid)}, {code}, {sql_str(r.get('CutterName'))}, {sql_num(r.get('Pitch'))}, {sql_date(r.get('CutterManufactureDate'))}) ")
            f.write(f"ON CONFLICT (cutter_code) DO UPDATE SET pitch = EXCLUDED.pitch, manufacture_date = EXCLUDED.manufacture_date;\n")
        ...
        for r in job_steps:
            ...
            f.write(f"({step_uuid(sid)}, {job_uuid(jid)}, {sid}, {item_id}, {status_id}, {sql_str(r.get('IDJobNo') or ('Step '+sid))}, {sql_date(r.get('ProcessingDeadline'))}, {sql_num(r.get('EstimatedHours'))}, {sql_str(r.get('Set'))}, {sql_str(r.get('IDCapDC'))}) ")
        ...
        for r in worklogs:
            wid = r.get("WorkLogID")
            sid = r.get("ProcessingDeadlineID")
            if not wid or not sid: continue
            
            emp = f"(SELECT employee_id FROM employees WHERE employee_code = 'EMP-' || {sql_str(r.get('EmployeeID', ''))} LIMIT 1)"
            f.write(f"INSERT INTO public.work_logs (log_id, job_step_id, employee_id, start_time, duration_hours, status, notes) VALUES ")
            dur = sql_num(r.get('ProcessingTime'))
            date = sql_date(r.get('ProcessingDate'))
            notes = sql_str(r.get('ProcessingNotes'))
            f.write(f"({worklog_uuid(wid)}, {step_uuid(sid)}, {emp}, {date}, {dur}, 'COMPLETED', {notes}) ")
