...
// --- CONFIGURATION ---
const ACCESS_DIR = path.join(process.cwd(), 'source_data/csv-access-data');
// For this script, we assume web CSVs are also downloaded into a web_csv_data folder or exist in the same folder if manually downloaded
// We'll look for web files in a specific folder or fallback to the same folder if they end with specific names.
const WEB_DIR = path.join(process.cwd(), 'source_data/web_data'); // Thư mục chứa file tải từ web
const OUTPUT_DIR = path.join(process.cwd(), 'source_data/merged_output');
...
async function mergeData() {
    ...
    const historyPath = path.join(ACCESS_DIR, 'datachangehistory.csv');
    let changes = [];
    ...
    const filesToCopy = ['moldmaster.csv', 'molddesign.csv', 'moldrevision.csv', 'tray.csv', 'companies.csv', 'customers.csv', 'worklog.csv'];
    let copyCount = 0;
