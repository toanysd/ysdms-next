...
async function run() {
  ...
  const jobsData: any[] = readFile('jobs.csv')
  const processingDeadlineData: any[] = readFile('processingdeadline.csv')
  const worklogData: any[] = readFile('worklog.csv')
  ...
  const designs = await fetchAll('design_revisions', 'revision_id, legacy_id, product_id, design_code')
  const accessDesignToUuid = new Map(designs.filter(d => d.legacy_id).map(d => [d.legacy_id, d.revision_id]))
  const designToProductUuid = new Map(designs.map(d => [d.revision_id, d.product_id]))
  ...
  const designCodeToMaster = new Map(designs.map(d => [d.design_code, d.product_id]))

  // 2b. Load products for fallback lookup by code
  const moldMasters = await fetchAll('products', 'product_id, product_code')
  const productCodeToId = new Map(moldMasters.map(m => [m.product_code, m.product_id]))
  ...
  for (const j of jobsData) {
    ...
    // Try to resolve design_revision_id and product_id through multiple strategies
    let design_revision_id = accessDesignToUuid.get(accessDesignId) || null
    let product_id = design_revision_id ? designToProductUuid.get(design_revision_id) : null

    // Strategy 2: Look up by design_code from the CSV
    if (!product_id && accessDesignId) {
      ...
      if (designCode) {
        ...
        product_id = designCodeToMaster.get(designCode) || null
      }
    }

    // Strategy 3: Look up mold_master directly by job code
    if (!product_id) {
      product_id = productCodeToId.get(jobCode) || null
    }

    // If still no product_id, skip with a warning
    if (!product_id) {
      skippedJobs++
      continue
    }
  ...
  for (const w of worklogData) {
    ...
    // Removed fallback to employees[0] to prevent assigning wrong logs to Yoshida
