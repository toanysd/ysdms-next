...
DO $$ DECLARE
    r RECORD;
BEGIN
    -- Step 1: Drop any YSDMS tables already in public (from previous partial runs)
    -- This avoids "already exists" errors when re-running
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename NOT ILIKE 'omni%') LOOP
        -- If same table exists in legacy_archive, drop public copy (it's a dup)
        IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'legacy_archive' AND tablename = r.tablename) THEN
            EXECUTE 'DROP TABLE IF EXISTS public.' || quote_ident(r.tablename) || ' CASCADE';
        ELSE
            EXECUTE 'ALTER TABLE public.' || quote_ident(r.tablename) || ' SET SCHEMA legacy_archive';
        END IF;
    END LOOP;

    FOR r IN (SELECT viewname FROM pg_views WHERE schemaname = 'public' AND viewname NOT ILIKE 'omni%') LOOP
        IF EXISTS (SELECT 1 FROM pg_views WHERE schemaname = 'legacy_archive' AND viewname = r.viewname) THEN
            EXECUTE 'DROP VIEW IF EXISTS public.' || quote_ident(r.viewname) || ' CASCADE';
        ELSE
            EXECUTE 'ALTER VIEW public.' || quote_ident(r.viewname) || ' SET SCHEMA legacy_archive';
        END IF;
    END LOOP;

    FOR r IN (SELECT t.typname FROM pg_type t JOIN pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = 'public' AND t.typtype = 'e' AND t.typname NOT ILIKE 'omni%') LOOP
        IF EXISTS (SELECT 1 FROM pg_type t2 JOIN pg_namespace n2 ON n2.oid = t2.typnamespace WHERE n2.nspname = 'legacy_archive' AND t2.typname = r.typname) THEN
            EXECUTE 'DROP TYPE IF EXISTS public.' || quote_ident(r.typname) || ' CASCADE';
        ELSE
            EXECUTE 'ALTER TYPE public.' || quote_ident(r.typname) || ' SET SCHEMA legacy_archive';
        END IF;
    END LOOP;

    FOR r IN (SELECT sequencename FROM pg_sequences WHERE schemaname = 'public' AND sequencename NOT ILIKE 'omni%') LOOP
        IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'legacy_archive' AND sequencename = r.sequencename) THEN
            EXECUTE 'DROP SEQUENCE IF EXISTS public.' || quote_ident(r.sequencename) || ' CASCADE';
        ELSE
            EXECUTE 'ALTER SEQUENCE public.' || quote_ident(r.sequencename) || ' SET SCHEMA legacy_archive';
        END IF;
    END LOOP;
END $$;
...
CREATE TABLE inventory_schedules (
    ...
    mold_owner_id     UUID REFERENCES customers(customer_id),
    schedule_type     TEXT,
...
CREATE TABLE sample_submissions (
    ...
    tracking_no       TEXT,
    customer_result   TEXT DEFAULT 'PENDING',
...
-- 69. customer_qr_labels
CREATE TABLE customer_qr_labels (
