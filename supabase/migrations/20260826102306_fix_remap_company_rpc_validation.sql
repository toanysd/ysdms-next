CREATE OR REPLACE FUNCTION public.remap_company_fks(
...
BEGIN
  -- 0. Validation
  IF p_old_company_id = p_new_company_id THEN
    RAISE EXCEPTION 'old and new company_id must differ';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM companies WHERE company_id = p_new_company_id AND is_active = true) THEN
    RAISE EXCEPTION 'target company_id does not exist or is inactive';
  END IF;
