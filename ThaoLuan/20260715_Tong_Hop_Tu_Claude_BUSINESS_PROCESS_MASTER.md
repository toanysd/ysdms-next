...
## YSDMS NextGen Business Process Catalog v1.1

**Ngày tổng hợp:** 2026-07-15  
**Cập nhật lần cuối:** 2026-07-15 v1.1 — Bổ sung chỉnh sửa từ Product Owner  
**Phương pháp:** Deep scan toàn bộ `source_data/` (830+ file Excel, 62+ CSV Access, 33 báo giá, 7 thư mục KH, ISO docs)  
**Đội ngũ:** 3 AI Scanner agents + AN tổng hợp + PO review
...
| Nhà máy hoạt động | **4+** (本社, 青森, 茨城, 坂田 — có thể liên quan thêm 日三化成) |
...
### 2.2 Quản lý khuôn & Thiết bị (金型・設備管理)

> [!IMPORTANT]
> **Lưu ý từ PO:** SACT là hệ thống riêng của Panasonic, KHÔNG liên quan trực tiếp. Các module di chuyển vị trí khuôn, vận chuyển khuôn, kiểm kê đã có sẵn trong hệ thống **MoldCutterSearch** đang hoạt động với UI/UX khá tốt. Source code hệ thống cũ này nằm trong `source_data/MoldCutterSearch_thamkhao/` (111 file JS+CSS).
...
| BP-22 | **Di chuyển vị trí khuôn** | Log mỗi lần di chuyển khuôn giữa các kệ | 1,488 log di chuyển | ✅ MoldCutterSearch (module vận chuyển) |
| BP-23 | **Kiểm kê khuôn (棚卸)** | Kiểm kê định kỳ, so sánh thực tế vs hệ thống | 373 log kiểm kê | ✅ MoldCutterSearch (module kiểm kê) |
...
| BP-28 | **Teflon coating** | Gửi khuôn phủ Teflon → 7 bước workflow | 4,680 lần, bao gồm chi phí | ✅ Module teflon riêng (teflonlog) |
| BP-29 | **Vận chuyển khuôn (出荷/入荷)** | Log giao/nhận khuôn giữa YSD và KH/nhà gia công | 349 log vận chuyển | ✅ Module vận chuyển khuôn |
| BP-30 | **Quản lý dao cắt (抜型管理)** | Đăng ký, gia công, mapping với khuôn | 1,724 dao + 2,730 mapping | ✅ cutters |

> [!IMPORTANT]
> **Bổ sung từ PO — Quản lý thiết bị toàn diện:**
> Dao cắt (cutters) chỉ là 1 loại thiết bị. Mục tiêu của ứng dụng là quản lý **TOÀN BỘ** các thiết bị phục vụ sản xuất, bao gồm:
> - **Dao cắt (抜型)**: Số lượng lớn, sản xuất cùng khuôn → mapping 1:N với khuôn
> - **Đế khí nén (圧空ベース)**: Dùng chung theo kích thước CAV
> - **Đế làm mát (水冷ベース)**: Dùng chung theo kích thước CAV
> - **Frame (フレーム)**: Dùng chung theo kích thước
> - **Stacking (スタッキング)**: Thiết bị xếp chồng
> - **Khuôn dập riêng (別抜き)**: Khuôn dập bổ sung
> - **Tấm mặt (面版)**: Tấm bề mặt
>
> **Yêu cầu nghiệp vụ:** Khi SX khay A → hệ thống phải chỉ ra NGAY tất cả thiết bị cần thiết (dao cắt, đế làm mát, khí nén...) kèm **vị trí lưu trữ** để nhân viên tìm kiếm nhanh chóng.
> Khi vận chuyển khuôn sang xưởng khác → phải biết cần chuyển **những thiết bị nào đi kèm**.
> Yêu cầu: quản lý linh hoạt, truy vết chính xác, lịch sử sử dụng & vận chuyển.
...
| # | Quy trình | Mô tả | Dữ liệu | Trạng thái |
|---|---|---|---|---|
| BP-34 | **Job gia công** | Cụ thể hóa các yêu cầu SX theo chỉ thị sản xuất. Thường là **tạo khuôn mới, kaizo (改造)** kèm yêu cầu hạn hoàn thành | 1,167 jobs, 37 trường | 🟡 jobs table |
| BP-35 | **Deadline gia công (công đoạn cấp 2)** | Là các **đối tượng cần xử lý** trong 1 job. VD: job SX khuôn cho đơn hàng mới → cần SX: khuôn nhôm, nắp khuôn gỗ, dao cắt, đế làm mát, đế khí nén... **Mỗi đối tượng có deadline riêng.** 13 trạng thái gia công | 2,403 deadline records | ⬜ Chưa đầy đủ |
| BP-36 | **44 loại công đoạn** | Chủ yếu là **gia công CNC** và các công đoạn hoàn thiện khuôn nhôm, khuôn gỗ. Hiện chưa có nghiệp vụ cho bộ phận SX khay/dập | 44 mã chuẩn | ⬜ Chưa import |
| BP-37 | **Nhật ký công việc (日報)** | Log hàng ngày: ai làm gì, bao lâu, công đoạn nào. **Mới xây dựng cho bộ phận SX khuôn**, bộ phận thiết kế chưa đầy đủ (có thể cung cấp thêm tài liệu thực tế) | 6,979 bản ghi, song ngữ JP/VN | 🟡 work_logs (bộ phận khuôn) |
...
| # | Quy trình | Mô tả | Edge Cases | Trạng thái |
|---|---|---|---|---|
| BP-40 | **Định hình (Thermoforming)** | Dùng máy ILLIG/Asano/Shanshin để tạo hình khay. Mỗi lần chạy máy cần 1 **SET thiết bị đầy đủ** theo kích thước khuôn × máy | Xem chi tiết SET thiết bị bên dưới | ⬜ Chưa |
| BP-41 | **Multi-site production** | SX tại 4+ nhà máy: 本社, 青森, 茨城, 坂田 (có thể liên quan thêm 日三化成) | Mỗi site có tồn kho + máy riêng | ⬜ Chưa |

> [!IMPORTANT]
> **Bổ sung từ PO — SET thiết bị cho máy định hình:**
> Mỗi lần định hình cần tập hợp đầy đủ các thiết bị tạo thành 1 SET cho từng loại kích thước khuôn theo từng máy:
>
> | Thiết bị | Tên JP | Mô tả |
> |---|---|---|
> | Khuôn (chính) | 金型 | Khuôn nhôm chính |
> | Dao cắt | 抜型 | Cắt hình sản phẩm |
> | Frame | フレーム | Khung giữ |
> | Đế khí nén | 圧空ベース | Base cho hệ thống khí nén |
> | Đế làm mát | 水冷ベース | Base cho hệ thống nước làm mát |
> | Stacking | スタッキング | Thiết bị xếp chồng sản phẩm |
> | Khuôn dập riêng | 別抜き | Khuôn dập bổ sung (nếu có) |
> | Tấm mặt | 面版 | Tấm bề mặt khuôn |
>
> Hệ thống YSDMS cần quản lý từng SET này, cho phép tra cứu nhanh toàn bộ thiết bị cần thiết khi chạy 1 đơn hàng cụ thể.
