    ...
    return (
        <div className="bg-white border border-blue-200 shadow-sm rounded-lg p-4 mt-4">
            <h3 className="text-sm font-bold text-blue-800 mb-3 flex items-center gap-2 border-b border-blue-100 pb-2">
                ...
                <span className="ja">生産指示 (Chỉ thị Sản xuất)</span>
            </h3>
                        ...
                        return (
                            <div key={item.id || index} className="border border-gray-200 rounded p-3 bg-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                <div className="flex-1 w-full">
                                    <div className="text-sm font-bold text-gray-800">{item.product_pn_raw || 'Chưa rõ mã mặt hàng'}</div>
                                    <div className="text-xs text-gray-500 mt-1">
                                        Yêu cầu: <span className="font-mono text-blue-600 font-bold">{item.quantity}</span> | 
                                        Đã lên lịch: <span className="font-mono text-green-600 font-bold">{totalPlanned}</span>
                                    </div>
                                        ...
                                        <div className="mt-2 space-y-1">
                                            {itemPlans.map(p => (
                                                <div key={p.id} className="text-[11px] flex gap-2 items-center bg-white border border-gray-200 px-2 py-1.5 rounded shadow-sm flex-wrap">
                                                    ...
                                                    <span className="text-gray-600 border-l pl-2 text-xs">{p.mold_physical?.physical_code || 'Chưa xếp khuôn'}</span>
                                                    <span className="ml-auto font-mono text-blue-600 font-bold bg-blue-50 px-1 rounded">Qty: {p.planned_quantity}</span>
                                ...
                                <div className="shrink-0 w-full md:w-auto">
                                    {!item.id ? (
                                        <span className="text-[10px] text-amber-600 bg-amber-50 px-2 py-1 rounded">Vui lòng lưu Đơn hàng trước</span>
                                    ) : (
                                        <button 
                                            type="button"
                                        ...
                                        >
                                            <Plus size={14} />
                                            <span className="ja">計画作成 (Tạo Lệnh)</span>
                                        </button>
                                    )}
                                </div>
                            </div>
