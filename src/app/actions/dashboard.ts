...
export async function getDashboardData() {
    ...
    const { data: inventory, error: invError } = await supabase
        .from('v_inventory_snapshot' as any)
    ...
    const inventoryData = (inventory || []) as unknown as Pick<InventorySnapshot, 'plastic_id' | 'current_stock_kg' | 'is_low_stock'>[]
    ...
    const ordersData = (ordersMonth || []) as unknown as Pick<OrderSummary, 'id' | 'status' | 'order_month'>[]
