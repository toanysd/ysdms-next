            ...
            <div className="p-6 max-w-xl mx-auto bg-[var(--mcs-surface)] min-h-screen">
                ...
                <div className="bg-[var(--mcs-surface-2)] p-4 rounded-lg border border-[var(--mcs-border)] mb-6">
                    <h1 className="text-xl font-bold text-[var(--mcs-text)] flex items-center gap-2 mb-2">
                        ...
                        Chốt Ca Máy (Hoàn thành)
                    </h1>
                    <div className="grid grid-cols-2 gap-4 text-sm mt-4">
                        <div><span className="text-[var(--mcs-text-muted)]">Mã hàng:</span> <b>{itemData.product_pn_raw}</b></div>
                        <div><span className="text-[var(--mcs-text-muted)]">Yêu cầu gốc:</span> <b>{itemData.quantity.toLocaleString()} pcs</b></div>
                        <div><span className="text-[var(--mcs-text-muted)]">Máy đang chạy:</span> <b>{logData.machine_master?.code}</b></div>
                        <div><span className="text-[var(--mcs-text-muted)]">Thợ máy:</span> <b>{logData.operator_name || 'N/A'}</b></div>
                        <div className="col-span-2"><span className="text-[var(--mcs-text-muted)]">Bắt đầu lúc:</span> <b>{new Date(logData.start_time).toLocaleString('vi-VN')}</b></div>
                    </div>
                </div>

                <form action={handleComplete} className="space-y-6">
                    <div>
                        <label className="block text-sm font-bold text-[var(--mcs-text)] mb-2">Số lượng THÀNH PHẨM (Good Qty)</label>
                        <input
                            type="number"
                            name="produced_qty"
                            required
                    ...
                    <div>
                        <label className="block text-sm font-bold text-[var(--mcs-text)] mb-2">Số lượng PHẾ PHẨM (Scrap/NG)</label>
                        <input
                            type="number"
                            name="scrap_qty"
                            defaultValue="0"
                    ...
                    <button type="submit" className="w-full h-16 text-xl font-bold bg-emerald-500 hover:bg-emerald-600 text-white rounded-md flex items-center justify-center gap-2 transition-colors shadow-lg">
                        <CheckCircle size={24} /> BÁO CÁO HOÀN THÀNH CA MÁY
                    </button>
                    <p className="text-xs text-center text-[var(--mcs-text-muted)] mt-2">Hệ thống sẽ tự nhận diện hao mòn khuôn vật lý nếu có.</p>
                </form>
            </div>
        )
    }
        ...
        <div className="p-6 max-w-xl mx-auto bg-[var(--mcs-surface)] min-h-screen">
            ...
            <div className="bg-[var(--mcs-surface-alert)] p-4 rounded-lg border border-[var(--mcs-border)] mb-6">
                <h1 className="text-xl font-bold text-[var(--mcs-text)] flex items-center gap-2 mb-2">
                    ...
                    Khởi động Ca Máy (Bắt đầu)
                </h1>
                <div className="text-sm mt-2 text-[var(--mcs-text-muted)]">
                    Bạn đang chuẩn bị mở ca sản xuất cho mã hàng <b className="text-[var(--mcs-text)]">{itemData.product_pn_raw}</b> (Yêu cầu: {itemData.quantity.toLocaleString()} pcs)
                </div>
            </div>
            ...
            <form action={handleStart} className="space-y-5">
                <div>
                    ...
                    <select name="machine_id" required className="w-full p-3 bg-[var(--mcs-surface)] border border-[var(--mcs-border)] rounded-md text-[var(--mcs-text)] focus:border-[var(--mcs-primary)] font-bold">
                        <option value="">-- Bấm để chọn Máy --</option>
                ...
                <div>
                    <label className="flex items-center gap-2 text-sm font-bold text-[var(--mcs-text)] mb-2">
                        ...
                        Tên Thợ (Người Vận Hành)
                    </label>
                    <input type="text" name="operator_name" placeholder="VD: Anh Tín, Chị Hoa..." className="w-full p-3 bg-[var(--mcs-surface)] border border-[var(--mcs-border)] rounded-md text-[var(--mcs-text)] focus:border-[var(--mcs-primary)]" />
                </div>

                <div>
                    <label className="flex items-center gap-2 text-sm font-bold text-[var(--mcs-text)] mb-2">
                        <Wrench size={16} className="text-[var(--mcs-primary)]" />
                        Gắn Mã Khuôn Vật Lý (Tùy chọn / Auto-resolve)
                    </label>
                    ...
                    {autoResolvedMold ? (
                        <div className="bg-emerald-500/10 border border-emerald-500 rounded-md p-4 mb-3">
                            <h3 className="font-bold text-emerald-600 flex items-center gap-2 mb-2">
                                <CheckCircle size={16} /> Khuôn được phân bổ tự động
                            </h3>
                            <div className="grid grid-cols-2 gap-2 text-sm">
                                <div><span className="text-[var(--mcs-text-muted)]">Mã khuôn:</span> <b className="text-[var(--mcs-text)]">{autoResolvedMold.physical_code} (Rev: {autoResolvedMold.revision_code})</b></div>
                                <div><span className="text-[var(--mcs-text-muted)]">Trạng thái:</span> <b className={autoResolvedMold.status === 'ACTIVE' ? "text-emerald-500" : "text-amber-500"}>{autoResolvedMold.status}</b></div>
                                <div className="col-span-2"><span className="text-[var(--mcs-text-muted)]">Vị trí kệ:</span> <b className="text-[var(--mcs-text)] font-mono text-lg px-2 py-0.5 bg-[var(--mcs-surface)] border border-[var(--mcs-border)] rounded shadow inline-block mt-1">{autoResolvedMold.storage_location || 'Chưa cấu hình kệ'}</b></div>
                            </div>
                        </div>
                    ) : (
                        <div className="bg-amber-500/10 border border-amber-500 rounded-md p-3 mb-3 text-sm text-amber-600 font-bold">
                            ⚠️ Khay này chưa được mapping với Khuôn nào trong hệ thống! Hoặc Khuôn chưa tạo thực thể vật lý.
                        </div>
                    )}
                    ...
                    <select name="mold_physical_id" defaultValue={autoResolvedMold?.id || ""} className="w-full p-3 bg-[var(--mcs-surface)] border border-[var(--mcs-border)] rounded-md text-[var(--mcs-text)] focus:border-[var(--mcs-primary)] font-bold">
                        <option value="">-- Không theo dõi Khuôn cho mã này --</option>
                        {availableMolds.map(m => <option key={m.id} value={m.id}>{m.physical_code} (Rev: {m.revision_code})</option>)}
                    </select>
                    <p className="text-[10px] text-[var(--mcs-text-muted)] mt-1">Cho phép thay đổi (override) chọn Khuôn khác nếu bộ phận bảo trì yêu cầu đổi khuôn.</p>
                </div>
                ...
                <div className="pt-4 border-t border-[var(--mcs-border)]">
                    <button type="submit" className="w-full h-14 text-lg font-bold bg-[var(--mcs-primary)] hover:bg-[var(--mcs-primary-dark)] text-white rounded-md flex items-center justify-center gap-2 transition-colors shadow">
                        <Play size={20} /> LÊN MÁY & BẮT ĐẦU CHẠY
                    </button>
                </div>
            </form>
        </div>
