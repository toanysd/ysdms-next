...
export default async function MachineMonitorPage() {
    ...
    const machineStatusMap = machines?.map(machine => {
        const job = activeLogs?.find((log: any) => log.machine_id === machine.id)
        return {
            ...machine,
            currentJob: job || null
        }
        ...
        <div className="flex-1 overflow-y-auto bg-[var(--mcs-surface)] min-h-screen">
            ...
            <header className="bg-[var(--mcs-surface-2)] p-4 border-b border-[var(--mcs-border)] flex justify-between items-center sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-[var(--mcs-primary)] flex items-center gap-2 tracking-wide uppercase">
                            <MonitorPlay size={28} />
                            FACTORY FLOOR MONITOR
                        </h1>
                        <p className="text-sm font-bold text-[var(--mcs-text-muted)] mt-1">
                            Live Machine Status • {new Date().toLocaleDateString('vi-VN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                        </p>
                    </div>
                </div>
                <div className="text-right">
                    ...
                    <div className="text-sm font-bold text-emerald-500 mt-1 uppercase">Running: {activeLogs?.length || 0} / {machines?.length || 0}</div>
                </div>
            </header>
                            ...
                            >
                                ...
                                <div className={`p-4 flex justify-between items-center ${isRunning ? 'bg-emerald-500 text-white' : 'bg-[var(--mcs-surface-2)] border-b border-[var(--mcs-border)]'}`}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h2 className={`text-2xl font-black ${isRunning ? 'text-white' : 'text-[var(--mcs-text)]'}`}>{machine.code}</h2>
                                            <p className={`text-xs font-bold uppercase ${isRunning ? 'text-emerald-100' : 'text-[var(--mcs-text-muted)]'}`}>{machine.name}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="p-5 flex-1 flex flex-col justify-center min-h-[140px]">
                                    {isRunning ? (
                                        <div className="space-y-4">
                                            <div>
                                                <div className="text-xs font-bold text-[var(--mcs-text-muted)] uppercase tracking-wider mb-1">Mã Sản Phẩm / Item</div>
                                            ...
                                            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-emerald-500/20">
                                                <div>
                                                    <div className="text-[10px] font-bold text-[var(--mcs-text-muted)] uppercase tracking-wider mb-1 flex items-center gap-1"><User size={12} /> Thợ Máy</div>
                                                    <div className="font-bold text-[var(--mcs-text)] truncate">{machine.currentJob.operator_name || 'N/A'}</div>
                                                </div>
                                                <div>
                                                    <div className="text-[10px] font-bold text-[var(--mcs-text-muted)] uppercase tracking-wider mb-1 flex items-center gap-1"><Clock size={12} /> Giờ Bắt Đầu</div>
                                        ...
                                        <div className="flex flex-col items-center justify-center text-center space-y-3 h-full">
                                            ...
                                            <p className="text-lg font-bold text-[var(--mcs-text-muted)] uppercase tracking-widest">Sẵn Sàng Hứng Tải</p>
                                        </div>
                                    )}
                                </div>
                            </div>
