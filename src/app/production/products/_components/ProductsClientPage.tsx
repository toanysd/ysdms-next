      ...
      <div className="flex-1 p-6 overflow-y-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {initialProducts.map(p => {
            const moldInfo = p.product_code || t('noCode')
            ...
            return (
              <div key={p.product_id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="text-[10px] px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded font-medium border border-indigo-100">{t('trayTag')}</div>
                </div>
                ...
                <div className="flex justify-between items-center text-xs text-slate-500 pt-3 border-t border-slate-100">
                  <div className="flex items-center gap-1"><span className="text-slate-400">{t('customerLabel')}</span> <span className="font-medium text-slate-700 truncate max-w-[80px]">{customerName}</span></div>
                  <div className="flex items-center gap-1"><span className="text-slate-400">{t('moldLabel')}</span> <span className="font-medium text-indigo-600">{moldInfo}</span></div>
                </div>
              </div>
            )
          })}
