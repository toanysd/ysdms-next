...
import {
  ArrowLeft, ArrowUpFromLine, Truck, Download, Save, Printer,
  Building2, MapPin, CheckCircle2, Clock, XCircle, AlertCircle,
  Package, Calendar, ExternalLink, RefreshCw, Layers, Boxes, FileText
} from 'lucide-react'
...
export default function ShipmentDetailPage({ params }: ShipmentDetailPageProps) {
  ...
  const loadShipment = async () => {
    ...
    setError(null)
    try {
      // 1. Fetch Shipment Header (Supporting both WO-direct and Order-based)
      const { data: sData, error: sErr } = await supabase
        .from('shipments')
        .select(`
          *,
          work_orders:work_orders!shipments_work_order_id_fkey (
            wo_id, wo_code, wo_name, wo_status,
      ...
      // 3. Fetch Linked Order Lines (Tiến độ Order Line nếu có order_id)
  ...
  const customerName = shipment.orders?.companies?.company_name ||
    shipment.work_orders?.companies?.company_name ||
    '得意先未設定'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, flexWrap: 'wrap', gap: 8, padding: '10px 14px', background: 'var(--bg-surface)', borderBottom: '1px solid var(--border-default)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <Link
            href={`/shipments/${shipment.shipment_id}/print`}
            target="_blank"
      ...
      <div className="card-flat" style={{ padding: 16, flexShrink: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
          ...
          {/* Linked Order & Work Order */}
          <div>
            <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, display: 'block', marginBottom: 2 }}>
              関連指示書・受注 (Linked Context)
            </span>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {shipment.work_orders && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span className="badge badge--neutral" style={{ fontSize: 10, fontFamily: 'monospace' }}>WO</span>
                  <Link
                    href={`/production/work-orders/${shipment.work_orders.wo_id}?tab=shipments`}
                    style={{
                      fontFamily: 'monospace', fontWeight: 800, fontSize: 13, color: 'var(--accent)',
                      textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4
                    }}
                  >
                    {shipment.work_orders.wo_code}
              ...
              {shipment.orders && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span className="badge badge--neutral" style={{ fontSize: 10, fontFamily: 'monospace' }}>ORD</span>
                  <Link
                    href={`/orders/${shipment.orders.order_id}`}
                    style={{
                      fontFamily: 'monospace', fontWeight: 800, fontSize: 13, color: '#0EA5E9',
                      textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4
                    }}
                  >
              ...
              {!shipment.work_orders && !shipment.orders && (
      ...
      {/* ── Section B: Chi tiết sản phẩm & Lô xuất kho ── */}
      <div className="card-flat" style={{ padding: 14 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span>出荷対象製品・明細 (Shipment Items & Lots)</span>
          </div>
          <span style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: 'monospace', fontWeight: 700 }}>
            総出荷数: {(Number(shipment.shipped_quantity) || 0).toLocaleString()} pcs
          </span>
        </div>
        ...
        {shipmentLots.length > 0 ? (
          ...
          <table className="data-table" style={{ width: '100%', fontSize: 12 }}>
            <thead>
              <tr>
                <th style={{ width: 140 }}>指示書番号 (WO)</th>
                <th style={{ width: 140 }}>製品コード</th>
                <th>製品名・指示書名</th>
                <th style={{ width: 130, textAlign: 'right' }}>出荷数量</th>
                <th style={{ width: 60, textAlign: 'center' }}>単位</th>
                <th style={{ width: 110, textAlign: 'center' }}>状態</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                <td style={{ fontFamily: 'monospace', fontWeight: 800 }}>
                  {shipment.work_orders.products?.product_code || '—'}
                </td>
                <td>
                  <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>
                    {shipment.work_orders.products?.product_name || shipment.work_orders.wo_name}
                  </span>
                </td>
                <td style={{ textAlign: 'right', fontFamily: 'monospace', fontWeight: 800, fontSize: 14, color: 'var(--text-primary)' }}>
                  {(Number(shipment.shipped_quantity) || 0).toLocaleString()}
                </td>
                <td style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
                  pcs
                </td>
      ...
      {/* ── Section C: Tiến độ Order Line hoặc Thông tin WO ── */}
      {orderLines.length > 0 ? (
        ...
        <div className="card-flat" style={{ padding: 16 }}>
          <h2 className="text-[13px] font-bold text-slate-700 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Package size={15} color="var(--accent)" />
            <span>製造指示書 情報 (Work Order Context)</span>
          </h2>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: '#F8FAFC', borderRadius: 6, border: '1px solid #E2E8F0' }}>
            <div>
              <span style={{ fontSize: 11, color: 'var(--text-muted)', display: 'block' }}>指示書名称</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{shipment.work_orders.wo_name}</span>
            </div>
            <div>
              <span style={{ fontSize: 11, color: 'var(--text-muted)', display: 'block' }}>状態</span>
              <span className="badge badge--success" style={{ fontSize: 11 }}>{shipment.work_orders.wo_status}</span>
            </div>
            <Link
              href={`/production/work-orders/${shipment.work_orders.wo_id}?tab=shipments`}
