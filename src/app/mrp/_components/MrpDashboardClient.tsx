      ...
      <header className="mb-6 flex justify-between items-center shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-[var(--mcs-text)] flex items-center gap-2">
            ...
            MRP 所要量計算 (Tính nhu cầu Vật tư)
          </h1>
          <p className="text-sm text-[var(--mcs-text-muted)]">Hệ thống tự động tính toán nhu cầu Cuộn nhựa dựa trên Đơn hàng đang chờ</p>
        </div>
      </header>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
        ...
        <div className="lg:col-span-2 flex flex-col bg-[var(--mcs-surface)] border border-[var(--mcs-border)] rounded-lg shadow-sm min-h-0">
          <div className="p-4 border-b border-[var(--mcs-border)] bg-[var(--mcs-surface-2)]">
            <h2 className="font-bold text-lg">Tổng hợp Nhu cầu & Tồn kho</h2>
          </div>
          <div className="flex-1 overflow-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-[var(--mcs-surface-2)] text-[var(--mcs-text-muted)] sticky top-0 z-10">
                <tr>
                  <th className="px-4 py-3 font-semibold">Mã Nhựa</th>
                  <th className="px-4 py-3 font-semibold text-right">Tồn Kho (m)</th>
                  <th className="px-4 py-3 font-semibold text-right">Tổng Nhu Cầu (m)</th>
                  <th className="px-4 py-3 font-semibold text-right">Thiếu Hụt (m)</th>
                ...
                {data.length === 0 && (
                  <tr>
                    <td colSpan={5} className="text-center py-10 text-[var(--mcs-text-muted)]">
                      ...
                      Không có nhu cầu vật tư (Không có Đơn hàng chờ)
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="flex flex-col bg-[var(--mcs-surface)] border border-[var(--mcs-border)] rounded-lg shadow-sm min-h-0">
          <div className="p-4 border-b border-[var(--mcs-border)] bg-[var(--mcs-surface-2)]">
            <h2 className="font-bold text-lg">Chi tiết Đơn hàng cấu thành</h2>
            {selectedData && (
              <p className="text-sm text-[var(--mcs-text-muted)] mt-1">Vật tư: {selectedData.plastic_code}</p>
            )}
          </div>
          <div className="flex-1 overflow-auto p-4">
            {!selectedData ? (
              <div className="h-full flex flex-col items-center justify-center text-[var(--mcs-text-muted)]">
                ...
                <p>Chọn một loại Nhựa để xem chi tiết</p>
              </div>
            ) : (
                ...
                {selectedData.demand_details.map((detail, idx) => (
                  <div key={idx} className="bg-[var(--mcs-surface-2)] border border-[var(--mcs-border)] rounded p-3">
                    <div className="flex justify-between items-start mb-2">
                      <div className="font-bold text-[13px] text-[var(--mcs-primary)]">ĐH: {detail.order_slip_no}</div>
                    ...
                    <div className="text-xs flex justify-between text-[var(--mcs-text-muted)]">
                      ...
                      <span>SL Đặt: {detail.qty_needed.toLocaleString()} khay</span>
                    </div>
                  </div>
                ))}
              </div>
