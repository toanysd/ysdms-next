      ...
      <div className="h-[48px] bg-teal-50 px-4 flex items-center justify-between border border-teal-200 rounded-t-lg shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[14px] font-bold text-teal-900 flex flex-col">
            {t('title')}
            <span className="text-teal-700 font-normal mt-[-2px] text-[10px]">{t('subtitle')}</span>
          </h2>
        </div>
      ...
      <div className="p-4 flex flex-col gap-4 border-x border-b border-teal-100 rounded-b-lg flex-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className={`bg-white border ${overdueCount > 0 ? 'border-red-300' : 'border-slate-200'} rounded-lg p-4 shadow-sm flex items-center justify-between`}>
            <div>
              <p className="text-xs text-red-600 font-bold mb-1 uppercase tracking-wide">{t('overdueCard')}</p>
              <h3 className="text-3xl font-black text-red-700">{overdueCount} <span className="text-sm font-normal text-red-500/70">{t('moldsUnit')}</span></h3>
            </div>
          ...
          <div className={`bg-white border ${warningCount > 0 ? 'border-amber-300' : 'border-slate-200'} rounded-lg p-4 shadow-sm flex items-center justify-between`}>
            <div>
              <p className="text-xs text-amber-600 font-bold mb-1 uppercase tracking-wide">{t('warningCard')}</p>
              <h3 className="text-3xl font-black text-amber-700">{warningCount} <span className="text-sm font-normal text-amber-600/70">{t('moldsUnit')}</span></h3>
            </div>
          ...
          <div className="bg-white border border-green-200 rounded-lg p-4 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs text-green-600 font-bold mb-1 uppercase tracking-wide">{t('okCard')}</p>
              <h3 className="text-3xl font-black text-green-700">{okCount} <span className="text-sm font-normal text-green-600/70">{t('moldsUnit')}</span></h3>
            </div>
        ...
        <div className="mt-4 bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
          <h3 className="text-sm font-bold text-slate-700 p-4 border-b">{t('allMoldsTitle')}</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead className="bg-[#f8fafc] text-xs text-slate-500">
                <tr>
                  <th className="p-3 font-bold border-b">{t('tableHeaders.physicalCode')}</th>
                  <th className="p-3 font-bold border-b">{t('tableHeaders.designCode')}</th>
                  <th className="p-3 text-right font-bold border-b w-[120px]">{t('tableHeaders.totalShots')}</th>
                  <th className="p-3 text-right font-bold border-b w-[120px]">{t('tableHeaders.interval')}</th>
                  <th className="p-3 border-b text-center w-[200px]">{t('tableHeaders.lifecycleProgress')}</th>
                  <th className="p-3 text-center font-bold border-b w-[100px]">{t('tableHeaders.health')}</th>
                </tr>
              </thead>
              <tbody className="text-[12px]">
                ...
                {healthList.length === 0 && (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-400 italic">{t('noPhysicalMolds')}</td>
                  </tr>
                )}
              </tbody>
            </table>
