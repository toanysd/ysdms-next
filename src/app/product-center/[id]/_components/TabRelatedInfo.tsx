...
import { Building2, ExternalLink, Briefcase, FileText, Layers } from 'lucide-react'
import Link from 'next/link'
import { MatchingMaterialStock } from './MatchingMaterialStock'
...
export function TabRelatedInfo({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {/* ── 1. Top Section: Matching Material Stock Card ── */}
      <MatchingMaterialStock productId={productId} />

      {/* ── 2. Bottom Section: Customer & Stakeholder Details ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 14 }}>

        {/* Customer Info Card */}
        <div className="card-flat" style={{ padding: 0, overflow: 'hidden', border: '1px solid var(--tint-blue-border, #bfdbfe)' }}>
          <div style={{
            background: 'var(--tint-blue-bg, #eff6ff)', borderBottom: '1px solid var(--tint-blue-border, #bfdbfe)',
            padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between'
          }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--tint-blue-text, #1e40af)', display: 'flex', alignItems: 'center', gap: 6 }}>
              <Building2 size={16} style={{ color: 'var(--tint-blue-text, #1e40af)' }} /> {t('mainCustomerInfoTitle')}
            </span>
            {customer && (
              <Link href={`/master/customers/${customer.company_id}`} style={{ fontSize: 11, color: 'var(--accent, #0D9488)', textDecoration: 'none', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 3 }}>
        ...
        <div className="card-flat" style={{ padding: 0, overflow: 'hidden', border: '1px solid var(--tint-teal-border, #99f6e4)' }}>
          <div style={{
            background: 'var(--tint-teal-bg, #f0fdfa)', borderBottom: '1px solid var(--tint-teal-border, #99f6e4)',
            padding: '10px 14px', fontSize: 13, fontWeight: 700, color: 'var(--tint-teal-text, #0f766e)',
            display: 'flex', alignItems: 'center', gap: 6
          }}>
            <Building2 size={16} style={{ color: 'var(--tint-teal-text, #0f766e)' }} /> {t('endUserInfoTitle')}
          </div>
        ...
        {/* Business Cases & Audit Notes Card */}
        <div className="card-flat" style={{ padding: 0, overflow: 'hidden', border: '1px solid var(--tint-orange-border, #fed7aa)' }}>
          <div style={{
            background: 'var(--tint-orange-bg, #fff7ed)', borderBottom: '1px solid var(--tint-orange-border, #fed7aa)',
            padding: '10px 14px', fontSize: 13, fontWeight: 700, color: 'var(--tint-orange-text, #c2410c)',
            display: 'flex', alignItems: 'center', gap: 6
          }}>
            <Briefcase size={16} style={{ color: 'var(--tint-orange-text, #c2410c)' }} /> {t('businessCasesAndAuditTitle')}
          </div>
